...
Процедура тестПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Тест_ДопПараметр") Тогда
		
		НовыйРеквизит = Новый РеквизитФормы("Тест_ДопПараметр", Новый ОписаниеТипов("Строка"));
		...
		ЭтаФорма.Тест_ДопПараметр = Параметры.Тест_ДопПараметр;
...
Функция тестПолучитьРабочийОбъект()
	...
	Попытка
		РабочийОбъект.ДополнительныеСвойства.Вставить("Тест_ДопПараметр", ЭтаФорма.Тест_ДопПараметр);
	Исключение
	КонецПопытки;	
