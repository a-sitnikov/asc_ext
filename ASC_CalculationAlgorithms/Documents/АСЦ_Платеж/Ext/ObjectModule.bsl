...
Процедура СконвертироватьДокумент() Экспорт
	...
	ДокументРТУ = ЗаполнитьНедостающиеСсылки();
	...
	Если ВидПлатежногоДокумента = "ПКО" 
		...
		СоздатьПКО(ДокументРТУ);
		
	ИначеЕсли ВидПлатежногоДокумента = "ППВ" Тогда 
		СоздатьПоступлениеДС(ДокументРТУ);
		
	ИначеЕсли ВидПлатежногоДокумента = "ПКО" 
		И СпособПлатежа = "Банковская карта" Тогда
		СоздатьОплатаПлатежнойКартой(ДокументРТУ);
	КонецЕсли;
		
КонецПроцедуры	

Функция ЗаполнитьНедостающиеСсылки() Экспорт
	
	СвойствоIDСчета = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя", "ID_Счета");
	ДокументРТУ = АСЦ_ПоискОбъектов.НайтиДокументПоСвойству(СсылкаСч, СвойствоIDСчета, "РеализацияТоваровУслуг");
	
	Если ЗначениеЗаполнено(ДокументРТУ) Тогда
		
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументРТУ, "Контрагент, ДоговорКонтрагента");
...
Процедура СоздатьПКО(ДокументРТУ)
...
Процедура СоздатьПоступлениеДС(ДокументРТУ)
...
Процедура СоздатьОплатаПлатежнойКартой(ДокументРТУ)
