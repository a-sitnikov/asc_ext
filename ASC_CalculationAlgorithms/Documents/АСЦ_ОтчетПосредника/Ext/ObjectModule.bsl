...
Функция НайтиДокументРТУПоИД(ДоговорКонтрагента, ИД)
	
	// В отчете может быть несколько документов. 
	// Ищем все, сортируем по сумме
...
Функция НайтиДокументРТУПоРеквизитам(ДоговорКонтрагента)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ АСЦ_ОтчетПосредника.Продажи КАК ОтчетПосредника
	|		ПО Док.Ссылка = ОтчетПосредника.Документ
	|			И ОтчетПосредника.Ссылка <> &ТекущийДокумент
	|ГДЕ
...
Функция НайтиДокументОтчетКомитенту(ИД)
	...
	|	Рег.Свойство = &Свойство
	|	И Рег.Значение = &ИД
	|	И Рег.Объект ССЫЛКА Документ.ОтчетКомитентуОПродажах";
...
Функция ЗаполнитьРТУПоСтрокамДоговора(МассивСтрок) Экспорт
КонецФункции	
...
Функция ЗаполнитьРТУПоСтроке(СтрокаТЧ) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ДокументСсылка) Тогда
		ДокументСсылка = НайтиДокументРТУПоИД(СтрокаТЧ.ДоговорКонтрагента, ID_Unicus);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ДокументСсылка) Тогда
		ДокументСсылка = НайтиДокументРТУПоРеквизитам(СтрокаТЧ.ДоговорКонтрагента);
	КонецЕсли;
...
Функция ЗаполнитьОтчетКомитенту() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ДокументСсылка) Тогда
		ДокументСсылка = НайтиДокументОтчетКомитенту(ID_Unicus);
	КонецЕсли;
