...
Процедура ЗаполнитьОтчет(ДокОбъект) Экспорт
	...
	|	V_ASC_CONTRACT tab
	|WHERE
	|	tab.DATE_SIGN >= TO_DATE(&ДатаНач, 'DD-MM-YYYY HH24:MI:SS')
	|	AND tab.DATE_SIGN <= TO_DATE(&ДатаКон, 'DD-MM-YYYY HH24:MI:SS')
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ДатаКон", "'" + Формат(КонецДня(ДокОбъект.ПериодОтчета.ДатаОкончания), "ДФ='dd.MM.yyyy HH:mm:ss'") + "'");
