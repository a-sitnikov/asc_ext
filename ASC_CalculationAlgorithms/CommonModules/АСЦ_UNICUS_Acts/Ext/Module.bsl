...
Процедура ЗаполнитьОтчет(ДокОбъект) Экспорт
	...
	Колонки = Новый Массив;
	Колонки.Добавить("Данные_СтраховаяПремия");
	Колонки.Добавить("Данные_КВ_руб");
	Колонки.Добавить("Данные_СуммаКПеречислению");
...
Функция ЗаполнитьАналитику(Таблица, Организация)
	...
	Результат.Колонки.Добавить("Аналитика1"); // Контрагент
	Результат.Колонки.Добавить("Аналитика2"); // Договор
	Результат.Колонки.Добавить("Аналитика3"); // Номенклатура 
	Результат.Колонки.Добавить("Аналитика4"); // Договор со страховой компанией
	Результат.Колонки.Добавить("Аналитика5"); // Акт ID - произвольное субконто
	
	МассивОшибок = Новый Массив;
	СубконтоАкт = ПланыВидовХарактеристик.ВидыСубконтоКорпоративные.НайтиПоНаименованию("UNICUS Акты");
	
	РодительФизЛица = Справочники.Контрагенты.НайтиПоНаименованию("1 Физические лица");
	
	Для каждого СтрокаТЗ из Результат Цикл
		
		ТекстОшибки = "";
		СтрокаТЗ.Аналитика1 = ПолучитьКонтрагента(СтрокаТЗ.Контрагент, СтрокаТЗ.КонтрагентИНН, СтрокаТЗ.КонтрагентКПП, ТекстОшибки, РодительФизЛица);
		...
		ТекстОшибки = "";
		КонтрагентСК = ПолучитьКонтрагента(СтрокаТЗ.СК, СтрокаТЗ.СК_ИНН, СтрокаТЗ.СК_КПП, ТекстОшибки);
		Если ЗначениеЗаполнено(ТекстОшибки)
			И МассивОшибок.Найти(ТекстОшибки) = Неопределено Тогда
			МассивОшибок.Добавить(ТекстОшибки);
		КонецЕсли;	
		
		Если ЗначениеЗаполнено(КонтрагентСК) Тогда
			
			ДоговорСК = ПолучитьДоговор(КонтрагентСК, Организация, СтрокаТЗ.НомерДок, Перечисления.ВидыДоговоровКонтрагентовУХ.СКомитентом);
			СтрокаТЗ.Аналитика4 = ДоговорСК;
			
		КонецЕсли;
		
		СтрокаТЗ.Аналитика5 = ПолучитьАкт(СубконтоАкт, СтрокаТЗ.ИД, СтрокаТЗ.ИД, "");
...
Функция ПолучитьКонтрагента(Наименование, ИНН, КПП, ТекстОшибки, Родитель = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(СпрСсылка) Тогда
		...
		СпрОбъект.ИНН          = ИНН;
		СпрОбъект.КПП          = КПП;
		Если СтрДлина(СпрОбъект.ИНН) = 10 Тогда
			...
			Если ЗначениеЗаполнено(КПП) Тогда
				...
				//ИдентификационныеНомераНалогоплательщиков.ЭтоПричинаПостановкиНаУчетОбособленногоПодразделения - не экспортная
				Если НЕ ИдентификационныеНомераНалогоплательщиков.ЭтоПричинаПостановкиНаУчетОрганизацииВЦелом(КодПричиныПостановкиНаУчет)
					И НЕ ИдентификационныеНомераНалогоплательщиков.ЭтоПричинаПостановкиНаУчетКрупнейшегоНалогоплательщика(КодПричиныПостановкиНаУчет) Тогда
					СпрОбъект.ОбособленноеПодразделение = Истина;
				КонецЕсли;
