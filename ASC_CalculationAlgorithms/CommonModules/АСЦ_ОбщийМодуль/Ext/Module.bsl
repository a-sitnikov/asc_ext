...
Функция ПолучитьДоговор(Данные, Перезаписывать, Основной = Ложь) Экспорт
	...
	//СпрОбъект.ОсновнаяСтатьяДвиженияДенежныхСредств = Данные.СтатьяДДС;
	
	Номенклатура = Неопределено;
	Данные.Свойство("Номенклатура", Номенклатура);
	Если ЗначениеЗаполнено(Номенклатура) Тогда
		
		РеквизитыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "Наименование, НоменклатурнаяГруппа.Наименование");
		КодСтатьи = АСЦ_UNICUS_PnL.ПолучитьКодСтатьи(РеквизитыНоменклатуры.Наименование, РеквизитыНоменклатуры.НоменклатурнаяГруппаНаименование);
		
		СоотвествиеСтатей = Новый Соответствие;
		СоотвествиеСтатей.Вставить("РозницаКАСКО",  "Страхование КАСКО новые а/м");
		СоотвествиеСтатей.Вставить("КорпКАСКО",     "Страхование корпоративных клиентов");
		СоотвествиеСтатей.Вставить("КомКАСКО",      "");
		СоотвествиеСтатей.Вставить("БУ_КАСКО",      "Страхование КАСКО а/м с пробегом");
		СоотвествиеСтатей.Вставить("ПролКАСКО",     "Пролонгация КАСКО");
		СоотвествиеСтатей.Вставить("РозницаОСАГО",  "Страхование ОСАГО новые а/м");
		СоотвествиеСтатей.Вставить("КорпОСАГО",     "Страхование корпоративных клиентов");
		СоотвествиеСтатей.Вставить("КомОСАГО",      "");
		СоотвествиеСтатей.Вставить("БУ_ОСАГО",      "Страхование ОСАГО  а/м с пробегом");
		СоотвествиеСтатей.Вставить("ПролОСАГО",     "Пролонгация ОСАГО");
		СоотвествиеСтатей.Вставить("СтрахЖизни",    "Страхование жизни");
		СоотвествиеСтатей.Вставить("ПролФинУслуги", "Пролонгация прочие финансовые услуги (РИНГ, GAP, СЖ)");
		СоотвествиеСтатей.Вставить("ПрочееСтрах",   "Прочее страхование (GAP)");
		СоотвествиеСтатей.Вставить("ПрочФинУслуги", "Прочие финансовые услуги (РИНГ, АСЦ-Помощь)");
		
		НаименованиеСтатьи = СоотвествиеСтатей[КодСтатьи];
		Если ЗначениеЗаполнено(НаименованиеСтатьи) Тогда
			СпрОбъект.ОсновнаяСтатьяИсполнение = Справочники.СтатьиДоходовИРасходов.НайтиПоНаименованию(НаименованиеСтатьи);
		КонецЕсли;	
