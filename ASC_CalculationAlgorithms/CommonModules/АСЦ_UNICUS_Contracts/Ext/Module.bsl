...
Процедура ЗаполнитьОтчет(ДокОбъект) Экспорт
	...
	|	TRUNC(tab.PAY_DATE) as PAY_DATE,
	...
	Для каждого СтрокаТЗ из Результат Цикл
		...
		Строка = СоответствиеСтрок[ВРег(СтрокаТЗ.Номенклатура)];
...
Функция ПолучитьСтруктуруСсылок(Данные, Параметры, База, КэшДанных, КэшСообщений, СтруктураСсылок = Неопределено) Экспорт
	...
	Если НЕ СтруктураСсылок.Свойство("Организация") Тогда
		СтруктураСсылок.Вставить("Организация", АСЦ_ОбщийМодуль.ПолучитьОрганизациюПоИНН(Данные.ОрганизацияИНН, КэшДанных.Организация));
	КонецЕсли;	
	
	Если НЕ ЗначениеЗаполнено(СтруктураСсылок.Организация) Тогда
		
		Если КэшСообщений.Организация[Данные.ОрганизацияИНН] = Неопределено Тогда
			
			ТекстОшибки = "Не  найдена организация: " + Данные.Организация + ", ИНН: " + Данные.ОрганизацияИНН;
			ЗаписьЖурналаРегистрации("Загрузка реализаций из Unicus",
				УровеньЖурналаРегистрации.Ошибка, , , ТекстОшибки);
			
			Сообщить(ТекстОшибки);
			КэшСообщений.Организация.Вставить(Данные.ОрганизацияИНН, Истина);
	...
	//ДанныеДоговора.Вставить("АналитикаИсполнение1", СтруктураСсылок.СК);
	//ДанныеДоговора.Вставить("АналитикаИсполнение2", АСЦ_ОбщийМодуль.НайтиБанк(Данные.БанкБИК));
