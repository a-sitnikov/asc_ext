...
Процедура ЗаполнитьОтчет(ДокОбъект) Экспорт
	...
	// Отбрасываем время
	|	TRUNC(tab.DATE_SIGN) as DATE_SIGN,
	|	TRUNC(tab.ACTION_BEGIN_DATE) as ACTION_BEGIN_DATE,
	|	TRUNC(tab.ACTION_END_DATE) as ACTION_END_DATE,
	// Страховая
	|	tab.SK_NAME as SK_NAME,
	// Подразделение
	|	tab.DEPARTMENT_NAME as DEPARTMENT_NAME,
	// ТС
	|	tab.VIN as VIN
	|FROM 
	|	V_ASC_CONTRACT tab
	...
	|	AND LEFT(tab.AGENT, 150) IN (&СписокОрганизаций)";
	...
	ТипБазы   = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокОбъект.ИспользуемаяИБ, "ТипБД"); 
	Настройка = Справочники.СоответствиеВнешнимИБ.НайтиПоНаименованию("AGENTS <-> Организации",,, ТипБазы);

	Запрос = Новый Запрос;
	Запрос.Параметры.Вставить("ОбъектТекущейИБ", ДокОбъект.Организация);
	Запрос.Параметры.Вставить("База",            ДокОбъект.ИспользуемаяИБ);
	...
	|ГДЕ
	|	Рег.ОбъектТекущейИБ = &ОбъектТекущейИБ
	|	И Рег.НастройкаСоответствия = &Настройка
	|	И Рег.ИспользуемаяИБ = &База";
	
	СписокОрганизаций = СтрСоединить(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ОбъектВнешнейИБ"), ", ");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СписокОрганизаций", СписокОрганизаций);
...
Функция ЗаполнитьАналитику(Таблица, Организация, База)
	...
	Кэш.Вставить("Организация",  Новый Соответствие);
	Кэш.Вставить("СК",           Новый Соответствие);
	...
	Настройка = Справочники.СоответствиеВнешнимИБ.НайтиПоНаименованию("DEPARTMENTS <-> Организации",,, ТипБазы);
	...
	Для каждого СтрокаТЗ из Результат Цикл
		...
		ПараметрыСК = ПолучитьПараметрыСтраховой(СтрокаТЗ.SK_NAME, Соединение, Кэш.СК);
		ТекстОшибки = "";
		СтрокаТЗ.Аналитика1 = АСЦ_ОбщийМодуль.НайтиКонтрагента(СтрокаТЗ.SK_NAME, ПараметрыСК.ИНН, ПараметрыСК.КПП, ТекстОшибки);
		
		Контрагент = ПолучитьКонтрагента(СтрокаТЗ.SUBJECT_NAME, Родитель);
		СтрокаТЗ.Аналитика2 = ПолучитьДоговор(Контрагент, Организация, СтрокаТЗ);
		СтрокаТЗ.Аналитика3 = ПолучитьНоменклатуру(СтрокаТЗ.PRODUCT_NAME, Кэш.Номенклатура);
		СтрокаТЗ.Аналитика4 = ПолучитьОрганизацию(СтрокаТЗ.DEPARTMENT_NAME, Настройка, База, Кэш.Организация);
...
Функция ПолучитьДоговор(Контрагент, Организация, Параметры)
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Если НЕ (Реквизиты.Дата = Параметры.DATE_SIGN
			И Реквизиты.ДатаНачала = Параметры.ACTION_BEGIN_DATE
			И Реквизиты.СрокДействия = Параметры.ACTION_END_DATE) Тогда
	...
	Иначе
		...
		СпрОбъект.ВидДоговораУХ = УправлениеДоговорамиУХКлиентСерверПовтИсп.ВидДоговораБП(СпрОбъект.ВидДоговораУХ);
...
Функция ПолучитьОрганизацию(Наименование, Настройка, База, Кэш)
...
Функция ПолучитьПараметрыСтраховой(Наименование, Соединение, Кэш)
	
	Параметры = Кэш[Наименование];
	Если Параметры <> Неопределено Тогда
		Возврат Параметры;
	КонецЕсли;	
	
	Параметры = Новый Структура("ИНН, КПП");
	
	// В таблице актов есть СК и ИНН/КПП.
	// возьмем из последнего акта.
	// наименование в шапке, ИНН/КПП - в таблице
	ТекстЗапроса = 
	"SELECT
	|    head.act_id as ИД,    
	...
	|FROM
	|	(SELECT 
	|        MAX(ACT_ID) as act_id
	|  	 FROM 
	|        v_asc_act
	|	 WHERE
	|		COL1 is NOT NULL
	|        AND COL4 = &Наименование) head
	|	LEFT JOIN v_asc_act tab 
	|		ON tab.act_id = head.act_id
	|        AND tab.COL1 IS NULL
	|WHERE
	|    rownum = 1";
	...
	Если НЕ НаборЗаписей.EOF Тогда		
		...
		Параметры.ИНН = НаборЗаписей.Fields("СК_ИНН").Value;
		Параметры.КПП = НаборЗаписей.Fields("СК_КПП").Value;
		
	КонецЕсли;	
	
	НаборЗаписей.Close();
	
	Кэш.Вставить(Наименование, Параметры);
	Возврат Параметры;
