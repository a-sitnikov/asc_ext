...
Процедура ЗаполнитьОтчет(ДокОбъект) Экспорт
	...
	// Организация - ЦФО
	ЦФО = АСЦ_ПоискОбъектов.ПолучитьЗначениеВнешнейИБ(ДокОбъект.Организация, ДопПараметры.НастройкаДепартаменты, ДокОбъект.ИспользуемаяИБ); 
	...
	Результат = ЗаполнитьАналитику(ТаблицаДанных, ДокОбъект.Организация, ДокОбъект.ИспользуемаяИБ, ДопПараметры);
	...
	Для каждого СтрокаТЗ из Результат Цикл
		...
		СтруктураСумм = ПолучитьСуммы(СтрокаТЗ);
		
		ДокОбъект.УстановитьЗначениеПоказателя("К" + СтатьяКод + "_СуммаОплаты", СтрокаТЗ.СуммаОплаты, 
			СтрокаТЗ.Аналитика1, СтрокаТЗ.Аналитика2, СтрокаТЗ.Аналитика3, СтрокаТЗ.Аналитика4, СтрокаТЗ.Аналитика5,, ЗаписатьИзменениеВЛог);
			
		ДокОбъект.УстановитьЗначениеПоказателя("К" + СтатьяКод + "_KV_RUB", СтруктураСумм.СуммаВыручка, 
			СтрокаТЗ.Аналитика1, СтрокаТЗ.Аналитика2, СтрокаТЗ.Аналитика3, СтрокаТЗ.Аналитика4, СтрокаТЗ.Аналитика5,, ЗаписатьИзменениеВЛог);
...
Функция ЗаполнитьАналитику(Таблица, Организация, База, ДопПараметры)
	...
	Для каждого СтрокаТЗ из Результат Цикл
		...
		СтрокаТЗ.Аналитика4    = СтруктураСсылок.Организация;
...
Функция ПолучитьСуммы(Данные) Экспорт
...
Функция ПолучитьСтруктуруСсылок(Данные, Параметры, База, КэшДанных, КэшСообщений) Экспорт
	...
	СтруктураСсылок.Вставить("БазаАстра",  ПолучитьБазуАстра(СтруктураСсылок.ЦФО, КэшДанных.БазаАстра));
	
	// Контрагент
	СтруктураСсылок.Вставить("Контрагент", АСЦ_ПоискОбъектов.ПолучитьКонтрагента(Данные, Параметры, Истина));
...
Функция ПолучитьБазуАстра(ЦФО, Кэш) Экспорт
