...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	Если ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		...
		НоваяКоманда.Представление = НСтр("ru = 'Выгрузка и загрузка данных XML'");
		НоваяКоманда.Идентификатор = "ВыгрузкаЗагрузкаДанныхXML";
...
Процедура ВыполнитьЗагрузку(Знач ИмяФайла) Экспорт
	...
	ТаблицаЗагруженныхСправочников=Новый ТаблицаЗначений;
	ТаблицаЗагруженныхСправочников.Колонки.Добавить("ТаблицаАналитики");
	ТаблицаЗагруженныхСправочников.Колонки.Добавить("Ссылка");
	ТаблицаЗагруженныхСправочников.Колонки.Добавить("Владелец");
	ТаблицаЗагруженныхСправочников.Колонки.Добавить("Наименование");
	ТаблицаЗагруженныхСправочников.Колонки.Добавить("Код");
	ТаблицаЗагруженныхСправочников.Колонки.Добавить("ЕстьВладелец",ОбщегоНазначенияУХ.ПолучитьОписаниеТиповБулево());
	
	////////////////////////////////////////////////////////////////////
	// Массив справочников, относящихся исключительно к настройке модели, в которых
	// не ожидается дублирующихся элементтов
	
	МассивИсключенийИмен=Новый Массив;
	
	МассивИсключенийИмен.Добавить("Справочник.ВидыОтчетов");
	МассивИсключенийИмен.Добавить("Справочник.БланкиОтчетов");
	МассивИсключенийИмен.Добавить("Справочник.ГруппыРаскрытия");
	МассивИсключенийИмен.Добавить("Справочник.КолонкиОтчетов");
	МассивИсключенийИмен.Добавить("Справочник.ОбластиДанныхВидовОтчетов");
	МассивИсключенийИмен.Добавить("Справочник.ПоказателиОтчетов");
	МассивИсключенийИмен.Добавить("Справочник.ПравилаОбработки");
	МассивИсключенийИмен.Добавить("Справочник.ПравилаПроверки");
	МассивИсключенийИмен.Добавить("Справочник.СтрокиОтчетов");
	МассивИсключенийИмен.Добавить("Справочник.ИсточникиДанныхДляРасчетов");
	...
	Пока Сериализатор.ВозможностьЧтенияXML(ЧтениеXML) Цикл
		...
		Попытка
			...
			МетаданныеОбъекта=ЗаписанноеЗначение.Метаданные();
			ИмяОбъекта= МетаданныеОбъекта.ПолноеИмя();
			
			Если (СтрНайти(ИмяОбъекта,"Справочник")>0 ИЛИ СтрНайти(ИмяОбъекта,"ПланВидовХарактеристик")>0)
				И МетаданныеОбъекта.ДлинаНаименования>0
				И (НЕ ЗаписанноеЗначение.Предопределенный)
				И МассивИсключенийИмен.Найти(ИмяОбъекта)=Неопределено Тогда
				
				НоваяСтрока=ТаблицаЗагруженныхСправочников.Добавить();
				НоваяСтрока.ТаблицаАналитики=ИмяОбъекта;
				НоваяСтрока.Ссылка=ЗаписанноеЗначение.Ссылка;
				
				Если СтрНайти(ИмяОбъекта,"Справочник")>0 Тогда
					
					НоваяСтрока.Владелец=ЗаписанноеЗначение.Владелец;
					НоваяСтрока.ЕстьВладелец=ЗначениеЗаполнено(НоваяСтрока.Владелец) И (НЕ НоваяСтрока.Владелец=Справочники.ТипыБазДанных.ТекущаяИБ);
					
					Если ИмяОбъекта="Справочник.СчетаБД" Тогда
						
						НоваяСтрока.Код=ЗаписанноеЗначение.Код;
	...
	ТаблицаЗагруженныхСправочников.Сортировать("ЕстьВладелец Убыв");
	
	МассивТекущаяИБ=ТаблицаЗагруженныхСправочников.НайтиСтроки(Новый Структура ("Владелец",Справочники.ТипыБазДанных.ТекущаяИБ));
	
	Для Каждого Строка ИЗ МассивТекущаяИБ Цикл
		
		ВыполнитьПроверкуДублей(Строка,ТаблицаЗагруженныхСправочников);
		ТаблицаЗагруженныхСправочников.Удалить(Строка);
		
	КонецЦикла;
	
	Для Каждого Строка ИЗ ТаблицаЗагруженныхСправочников Цикл
...
Процедура ВыполнитьПроверкуДублей(Строка,ТаблицаЗагруженныхСправочников)
	
	Запрос=Новый Запрос;
	
	Если Строка.ТаблицаАналитики="Справочник.СчетаБД" Тогда
		
		Запрос.Текст="Выбрать ТаблицаСправочник.Ссылка Как Ссылка ИЗ "+Строка.ТаблицаАналитики+ " КАК ТаблицаСправочник
		|ГДЕ ТаблицаСправочник.Код=&Код
		|И НЕ ТаблицаСправочник.Ссылка=&Ссылка";
	...
	Иначе
		...
		Запрос.Текст="Выбрать ТаблицаСправочник.Ссылка Как Ссылка ИЗ "+Строка.ТаблицаАналитики+ " КАК ТаблицаСправочник
		|ГДЕ ТаблицаСправочник.Наименование=&Наименование
		|И НЕ ТаблицаСправочник.Ссылка=&Ссылка";
	...
	Если ЗначениеЗаполнено(Строка.Владелец) Тогда
		
		Запрос.Текст=Запрос.Текст+"
		|И ТаблицаСправочник.Владелец=&Владелец";
	...
	Если Результат.Следующий() Тогда
		...
		МассивДублей.Добавить(Строка.Ссылка);
		
		РезультатЗамены=ОбщегоНазначенияУХ.ВыполнитьЗаменуЭлементов(Результат.Ссылка,МассивДублей,Ложь);
		
		Если РезультатЗамены Тогда
			
			МассивПодчиненных=ТаблицаЗагруженныхСправочников.НайтиСтроки(Новый Структура("Владелец",Строка.Ссылка));
			
			Для Каждого СтрПодчиненный ИЗ МассивПодчиненных Цикл
				
				СтрПодчиненный.Владелец=Результат.Ссылка;
...
КонецПроцедуры // ВыполнитьПроверкуДублей()
...
Функция ПодготовитьПостроительДляВыгрузки(СтрокаДереваМетаданных, СтрокаПолейДляВыборки = "") Экспорт
	...
	Фильтр = ФильтрПоискаПоКомпоновщику(СтрокаДереваМетаданных.Отбор);
	
	ДобавитьУсловиеГде = Найти(ВРег(ИтоговыйТекстЗапроса), "ГДЕ") = 0;
	
	ИтоговыйТекстЗапроса = ИтоговыйТекстЗапроса + " 
	    |	" + ?(ПустаяСтрока(Фильтр.Текст), "", ?(ДобавитьУсловиеГде, " ГДЕ ", " И ")  + "(" + Фильтр.Текст + ")");
...
//
Функция ФильтрПоискаПоКомпоновщику(Знач ОтборКомпоновщика)
	...
	Результат.Вставить("Текст", ТекстОтбораГруппыКомпоновщика(ОтборКомпоновщика.Элементы, СтекГрупп, Результат.Параметры) );
	Результат.Вставить("Описание", Строка(ОтборКомпоновщика) );
