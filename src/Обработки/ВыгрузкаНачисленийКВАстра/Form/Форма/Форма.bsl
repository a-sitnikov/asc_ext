...
Процедура ЗарегистрироватьНаСервере()
	...
	Для каждого СтрокаТЗ из ТаблицаРегистрации Цикл
		...
		Запись.НастройкаСоответствия = Справочники.СоответствиеВнешнимИБ.НайтиПоНаименованию("КВ банков");
...
Функция ПоказатьЗапросНаСервере()
	...
	Данные = АСЦ_ВыгрузкаВАстра.ПолучитьДанныеДокумента_КВбанков(ОтборДокумент);
	ТекстЗапроса = АСЦ_ВыгрузкаВАстра.ПолучитьТекстЗапроса_КВБанков(Данные, База, Кэш);
