...
Функция ВыгрузитьДокументыСервер()
	...
	СтруктураПараметров.Вставить("Тест_Документ", Тест_Документ);
...
Процедура ПоказатьЗапрос(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Тест_Документ) Тогда
...
Функция ПоказатьЗапросНаСервере()
	...
	ТекстЗапроса = ОбработкаОбъект.ПолучитьТекстЗапроса(Тест_Документ, База, ДопПараметры, Кэш);
