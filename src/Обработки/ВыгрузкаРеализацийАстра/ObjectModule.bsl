...
Процедура ВыгрузитьДокументы(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК Док1
	|		ПО Рег.Элемент = Док1.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаРеализации КАК Док2
...
Функция ПолучитьДанныеДокумента(Ссылка, Параметры)
	...
	Если ЗначениеЗаполнено(СтрокаТЗ.Статья) Тогда
		Результат.Вставить("Статья",         Строка(СтрокаТЗ.Статья.УникальныйИдентификатор()));
	Иначе	
		Результат.Вставить("Статья",         Неопределено);
	КонецЕсли;	
	...
	Результат.Вставить("VIN",            СокрЛП(СтрокаТЗ.VIN));
	Результат.Вставить("Номенклатура",   СтрокаТЗ.Номенклатура);
	
	Если НЕ ЗначениеЗаполнено(СтрокаТЗ.IDПроекта)
		ИЛИ СтрокаТЗ.IDПроекта = "nodata" Тогда
...
Функция ПолучитьТекстЗапроса(ДокументСсылка, База, Параметры, Кэш) Экспорт
	
	Данные = ПолучитьДанныеДокумента(ДокументСсылка, Параметры);
	Если Данные.Статья = Неопределено Тогда
		ТекстОшибки = "В договоре " + Данные.Договор + ", для номенклатуры: " + Данные.Номенклатура + " не указана статья";
		Сообщить(ТекстОшибки);
		ЗаписьЖурналаРегистрации("Выгрузка ОФУ в Астра",
...
Процедура ВыгрузитьДокумент(Ссылка, База, Параметры, Кэш)
	...
	ТекстЗапроса = ПолучитьТекстЗапроса(Ссылка, База, Параметры, Кэш);
