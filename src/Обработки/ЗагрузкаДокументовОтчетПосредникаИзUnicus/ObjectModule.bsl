...
Процедура ЗагрузитьОтчеты(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	IDПоследнегоДокумента  = Параметры.IDПоследнегоДокумента;
	...
	Если ЗначениеЗаполнено(IDПоследнегоДокумента) Тогда
		УсловиеДата = "ACT_ID > " + Формат(IDПоследнегоДокумента, "ЧГ=0");
	Иначе	
		
		УсловиеДата = "TO_DATE(COL3, 'DD.MM.YYYY') BETWEEN &ДатаНач AND &ДатаКон";
	...
	Для каждого СтрокаТЗ из ТаблицаДанных Цикл
		...
		Если ЗначенияОтличаются(ПредЗначения, СтрокаТЗ) Тогда
			...
			Если ДокОбъект <> Неопределено Тогда
				...
				ЗаписатьДокумент(ДокОбъект, РежимЗаписиДокумента.Запись,, ОтложенноеПроведение);
			КонецЕсли;	
