...
Процедура УстановитьВидимость(ЭтаФорма)
	...
	Если ЭтаФорма.Вариантзагрузки = "Период" Тогда
		...
		ЭтаФорма.Элементы.IdПоследнегоДокумента.Видимость = Ложь;
	Иначе
		ЭтаФорма.Элементы.Период.Видимость = Ложь;
		ЭтаФорма.Элементы.IdПоследнегоДокумента.Видимость = Истина;
	КонецЕсли;	
...
Функция ЗагрузитьОтчетыСервер()
	...
	СтруктураПараметров.Вставить("IDПоследнегоДокумента",  IdПоследнегоДокумента);
