...
Процедура Загрузить() Экспорт
	...
	Для каждого СтрокаТЗ из Таблица Цикл
		...
		Организация = ПолучитьОрганизацию(СтрокаТЗ.AGENT);
		Если Не ЗначениеЗаполнено(Организация) Тогда
			Сообщить("Не найдена организация: " + СтрокаТЗ.AGENT);
			Продолжить;
		КонецЕсли;	
		
		ДокументСсылка = ПолучитьЭкземплярОтчета(Организация, ВидОтчета, Сценарий);
		...
		ДокументОбъект.ДополнительныеСвойства.Вставить("Тест_ДопПараметр", "AND rownum <= 10");
		
		ДокументОбъект.ИнициализироватьКонтекст();
		Если НЕ ЗначениеЗаполнено(ДокументСсылка) Тогда
			ДокументОбъект.ОчиститьВсе();
		КонецЕсли;	
...
Функция ПолучитьЭкземплярОтчета(Организация, ВидОтчета, Сценарий)
