...
Процедура ЗагрузитьContracts() Экспорт
	...
	Для каждого СтрокаТЗ из Таблица Цикл
		...
		текОрганизация = АСЦ_ОбщийМодуль.ПолучитьОрганизациюПоИНН(СтрокаТЗ.ОрганизацияИНН, СтрокаТЗ.ОрганизацияКПП, КэшДанных);
		
		Если Не ЗначениеЗаполнено(текОрганизация) Тогда
			ОшибкиОрганизация = + ОшибкиОрганизация + Символы.ПС + 
			СтрокаТЗ.Организация + ", ИНН: " + СтрокаТЗ.ОрганизацияИНН + ", КПП: " + СтрокаТЗ.ОрганизацияКПП;
			Продолжить;
		КонецЕсли;	
		
		Сообщить(текОрганизация);
