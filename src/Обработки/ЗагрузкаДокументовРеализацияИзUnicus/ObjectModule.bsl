...
Функция ПолучитьТаблицуДанныхРеализации(База, ДатаНач, ДатаКон, Параметры ) Экспорт
	...
	Если Параметры.Свойство("ОтборДоговор") Тогда
		...
		|	tab.Договор = '" + СокрЛП(Параметры.ОтборДоговор) + "'";
	КонецЕсли;	
			...
			Иначе
				...
				Иначе	
					...
					Тогда
						...
						ЗаполнитьДокументСторно(ДокументРТУ, Данные, Параметры, СтруктураСсылок);
						
						Данные.Дата = Данные.ДатаИзменения;
						ДокументСсылка = ЗаполнитьДокументРТУ(Неопределено, Данные, Параметры, СтруктураСсылок);
