...
Функция ПолучитьСтруктуруСсылок(Данные, Параметры, База, КэшДанных, КэшСообщений)
...
Функция ПолучитьТаблицуДанныхРеализации(База, ДатаНач, ДатаКон, ОтборОрганизация = Неопределено, Тест_КоличествоСтрок = Неопределено, ОтборДоговор = Неопределено, ОтборVIN = Неопределено)
	...
	|	OR tab.PAY_DATE BETWEEN &ДатаНач AND &ДатаКон)";
	...
	Если ЗначениеЗаполнено(ОтборДоговор) Тогда
		...
		|		THEN REPLACE(tab.POLICY_NUMBER, ' ', '')
		|		ELSE REPLACE(tab.POLICY_SERIA, ' ', '') || REPLACE(tab.POLICY_NUMBER, ' ', '')
		|	END = '" + СокрЛП(ОтборДоговор) + "'";
	КонецЕсли;	
		
	Если ЗначениеЗаполнено(ОтборVIN) Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|	AND tab.VIN = '" + СокрЛП(ОтборVIN) + "'";
	КонецЕсли;	
...
Процедура ЗагрузитьРеализации(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	ОтборVIN = "";
	Параметры.Свойство("ОтборVIN", ОтборVIN);
	...
	ТаблицаДанных = ПолучитьТаблицуДанныхРеализации(База, ДатаНач, ДатаКон, ОтборОрганизация, Тест_КоличествоСтрок, ОтборДоговор, ОтборVIN);
...
Функция СоздатьДокументРТУ_Корректировка_Сторно(Данные, Параметры, СтруктураСсылок, КэшДанных)
	...
	ОтчетПосредника = ПолучитьОтчетПосредника(ДокументСсылка);
...
Функция ЗаполнитьДокументРТУ(ДокументСсылка, Данные, Параметры, СтруктураСсылок)
	...
	Если Данные.ТипПлатежа = "Прямые расчеты с СК"
		...
		ДокументОбъект.Комментарий = Данные.ТипПлатежа;
	КонецЕсли;	
...
Функция ЗаполнитьДокументСторно(ДокументРТУ, Данные, Параметры, СтруктураСсылок)
	...
	ЗаписатьДокумент(ДокументОбъект, РежимЗаписиДокумента.Запись, Параметры.ВыводитьСообщения, Ложь);		
	ДобавитьИзменениеОбъекта(ДокументОбъект.Ссылка, СтруктураСсылок.БазаАстра);
