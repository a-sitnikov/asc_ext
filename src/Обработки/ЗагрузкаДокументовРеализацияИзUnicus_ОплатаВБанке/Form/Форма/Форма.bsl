...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяТаблицы    = "unicusweb.v_asc_pay_contract_hist";
...
Процедура УстановитьТекстГруппыОтбора(ЭтаФорма)
	...
	Если ЭтаФорма.ОтборПродуктИспользование Тогда
		МассивСтрок.Добавить("продукт: " + ЭтаФорма.ОтборПродукт);
	КонецЕсли;	
...
&НаКлиенте
Процедура ОтборПродуктПриИзменении(Элемент)
	
	ОтборПродуктИспользование = Истина;
	УстановитьТекстГруппыОтбора(ЭтаФорма);
...
Процедура ОбновитьДатуНаСервере()
	...
	Попытка
		ДатаПоследненгоДокумента = ХранилищеОбщихНастроек.Загрузить("ЗагрузкаДокументовИзUnicus", "ДатаПоследнегоДокументаОплатаВБанке",, "ЗагрузкаДокументовИзUnicus");
	Исключение
	КонецПопытки;
...
Функция ПолучитьСтруктуруПараметров()
	...
	Если ОтборПродуктИспользование Тогда
		СтруктураПараметров.Вставить("ОтборПродукт",      ОтборПродукт);
	КонецЕсли;
...
Функция ПоказатьЗапросНаСервере()
	...
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	СтруктураПараметров = ПолучитьСтруктуруПараметров();
	ТекстЗапроса = ОбработкаОбъект.ПолучитьТекстЗапроса(СтруктураПараметров.ДатаНач, СтруктураПараметров.ДатаКон, СтруктураПараметров);
