...
Процедура ЗавершениеЗагрузки(АдресРезультата)
	...
	Объект.СозданныеДокументы.Загрузить(ТаблицаДанных);
...
Процедура ПолучитьДокументы(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьДокументыОтвет", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, "Получить документы?", РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет);
	
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьДокументыОтвет(Ответ, ДополнительныеПараметры) Экспорт
	...
	Задание = ПолучитьДокументыСервер();
...
Функция ПолучитьДокументыСервер()
	...
	Если ВыполнятьвФоне Тогда
		...
		ПараметрыЗадания.Вставить("ИмяМетода",                     "ПолучитьДокументы");
	...
	Иначе
		...
		ОбработкаОбъект.ПолучитьДокументы(СтруктураПараметров, АдресРезультата);
