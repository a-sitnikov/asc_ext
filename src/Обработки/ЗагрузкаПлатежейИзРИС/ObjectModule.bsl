...
Процедура ЗагрузитьПлатежи(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	// Разбиваем по дням, ибо документов очень много
	текДата = Параметры.ДатаНач;
	Пока текДата < Параметры.ДатаКон Цикл
	
		ЗагрузитьПлатежиЗаПериод(WSПрокси, текДата, КонецДня(текДата), Параметры, СчетчикДокументов);
...
Процедура ЗагрузитьПлатежиЗаПериод(WSПрокси, ДатаНач, ДатаКон, Параметры, СчетчикДокументов) Экспорт
	
	Организация = Параметры.Организация;
	
	ВозвращаемПлатежки    = Ложь;
	ДЦВыпискиДляПроверки  = Неопределено;
	ТаблицаСчетов         = Неопределено;
	ТаблицаДокументов     = Неопределено;
	ТаблицаСчетов         = Неопределено;
	ЦБОтправленныеВыписки = Неопределено;
	ТекстОшибки = "";
	
	ДлительныеОперации.СообщитьПрогресс(0,  Формат(ДатаНач, "ДФ=dd.MM.yyyy") + ", получение данных");
	
	Ответ = WSПрокси.InvoiceGet(70,  // Код Юникуса в РИС
		"",
		ДатаНач,
		ДатаКон,
		ВозвращаемПлатежки,
		ДЦВыпискиДляПроверки,
		ТаблицаСчетов,
		ТаблицаДокументов,
		ТекстОшибки,
		ЦБОтправленныеВыписки);
	...
	Для каждого СтрПлатеж Из ТаблицаДокументов.Платеж Цикл
		...
		ДлительныеОперации.СообщитьПрогресс(ПроцентВыполнения, Формат(ДатаНач, "ДФ=dd.MM.yyyy") + ", обработано " + Счетчик + " из " + Всего);
		...
		ДокументСсылка = Документы.АСЦ_Платеж.НайтиПоНомеру(СтрПлатеж.Номер, ДатаНач);
...
Функция ЗаполнитьДокументПлатеж(ДокументСсылка, СтрПлатеж, WSПрокси, СтруктураСсылок, Параметры)
	...
	Если Параметры.ВыводитьСообщения = Истина Тогда
		Сообщение = Новый СообщениеПользователю;
