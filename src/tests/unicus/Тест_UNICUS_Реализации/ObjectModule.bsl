...
Процедура ЗаполнитьНаборТестов(НаборТестов) Экспорт
	...
	НаборТестов.Добавить("РТУ_ИзменениеТипаПлатежа");
	...
	НаборТестов.Добавить("РТУ_ИзменениеЦентра_ОтчетПосредника");
	НаборТестов.Добавить("РТУ_ЗакрытыйПериод");
...
Процедура Реализация() Экспорт
	...
	ДокументСсылка = ОбработкаОбъект.НайтиДокументНачислениеКВ("999999999999");
	
	Утверждения.ПроверитьЗаполненность(ДокументСсылка, "Документ ""Плановое  начисление КВ"" не создан");
...
Процедура РТУ_ИзменениеЦентра() Экспорт
	...
	ДЦ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ДЦ");
...
Процедура РТУ_ИзменениеНоменклатуры() Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	Док.Номенклатура.Наименование КАК Номенклатура,
...
Процедура РТУ_ИзменениеТипаПлатежа() Экспорт
	
	Макет = ПолучитьМакет("РТУ_ИзменениеТипаПлатежа");
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "ВидОперации, Сумма");
	Утверждения.ПроверитьРавенство(Реквизиты.ВидОперации, Справочники.АСЦ_ВидыОперацийПлановоеНачислениеКВ.ПрямыеРасчетыССК,  "Не верный вид операции");
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "ВидОперации, Сумма");
	Утверждения.ПроверитьРавенство(Реквизиты.ВидОперации, Справочники.АСЦ_ВидыОперацийПлановоеНачислениеКВ.Страховки,  "Не верный вид операции");
	Утверждения.ПроверитьРавенство(Реквизиты.Сумма,       11200.96,  "Не верная сумма документа");
...
Процедура РТУ_ИзменениеЦентра_ДатаЗапрета() Экспорт
	...
	ДокументРТУ1 = ОбработкаОбъект.НайтиДокументНачислениеКВ("999999999999");
	Утверждения.ПроверитьЗаполненность(ДокументРТУ1, "Документ ""Плановое  начисление КВ"" не создан");
	
	ДЦ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументРТУ1, "ДЦ");
	Утверждения.ПроверитьРавенство(Строка(ДЦ), "КИА-Север",  "Не верный ДЦ");
	...
	ДокументРТУ2 = ОбработкаОбъект.НайтиДокументНачислениеКВ("999999999999");
	Утверждения.ПроверитьНеРавенство(ДокументРТУ1, ДокументРТУ2, "Новый документ РТУ не создан");
	
	ДЦ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументРТУ2, "ДЦ");
	Утверждения.ПроверитьРавенство(Строка(ДЦ), "Инфинити-Химки",  "Не верный ДЦ");
...
Процедура РТУ_ИзменениеДоговора_ДатаЗапрета() Экспорт
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументРТУ2, "ДоговорКонтрагента, Основание, Дата");
	Утверждения.ПроверитьРавенство(Строка(Реквизиты.ДоговорКонтрагента), "TEST-00002",  "Не верный договор");
	Утверждения.ПроверитьРавенство(Реквизиты.Основание, ДокументРТУ1,  "Не верный документ основание");
	...
	ДокументРТУ3 = ОбработкаОбъект.НайтиДокументНачислениеКВ("999999999999");
	Утверждения.ПроверитьРавенство(ДокументРТУ3, ДокументРТУ2, "Ошибочно создан новый документ РТУ");
	
	СуммаКВ = ДокументРТУ2.СуммаКВ; 
...
Процедура РТУ_ОтвязкаПлатежа() Экспорт
	...
	СуммаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументРТУ1, "Сумма");
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументРТУ1, "ДоговорКонтрагента, Сумма");
	Утверждения.ПроверитьРавенство(Строка(Реквизиты.ДоговорКонтрагента), "TEST-00002", "Не верный договор");
	Утверждения.ПроверитьРавенство(Реквизиты.Сумма, 11200.96, "Не верная сумма");
...
Процедура РТУ_ОтвязкаПлатежа_ДатаЗапрета() Экспорт
	...
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументРТУ2, "ДоговорКонтрагента, Сумма");
	Утверждения.ПроверитьРавенство(Строка(Реквизиты.ДоговорКонтрагента), "TEST-00002", "Не верный договор");
	Утверждения.ПроверитьРавенство(Реквизиты.Сумма, 11200.96, "Не верная сумма");
	
КонецПроцедуры

Процедура РТУ_ИзменениеЦентра_ОтчетПосредника() Экспорт
	
	Макет = ПолучитьМакет("РТУ_ИзменениеЦентра_ОтчетПосредника");
	...
	//Отчет посредника 28.02
	Каталог = "E:\Ситников\asc_ext\Обработки\";
	ОбработкаОтчетыПосредника = ВнешниеОбработки.Создать(Каталог + "ЗагрузкаДокументовОтчетПосредникаИзUnicus.epf", Ложь);
	
	ТаблицаДанных = ОбработкаОбъект.ПолучитьТаблицуДанныхРеализацииТест(Макет.ПолучитьОбласть("ОтчетПосредника"));
	
	ПараметрыОтчетыПосредника = Новый Структура;
	ПараметрыОтчетыПосредника.Вставить("ДатаНач", '2018-02-01');
	ПараметрыОтчетыПосредника.Вставить("ДатаКон", '2018-02-28');
	ПараметрыОтчетыПосредника.Вставить("База",    Справочники.ВнешниеИнформационныеБазы.НайтиПоНаименованию("Юникус"));
	
	ПараметрыОтчетыПосредника.Вставить("ПерезаполнятьДокументы",   Ложь);
	ПараметрыОтчетыПосредника.Вставить("ВыводитьСообщения",        Ложь);
	ПараметрыОтчетыПосредника.Вставить("ОтложенноеПроведение",     Истина);
	
	АдресРезультата = Неопределено;
	
	ОбработкаОтчетыПосредника.ЗагрузитьОтчеты(ПараметрыОтчетыПосредника, АдресРезультата, ТаблицаДанных);
	ОбработкаОтчетыПосредника = Неопределено;
...
Процедура РТУ_ЗакрытыйПериод() Экспорт
	
	// Закрываем период
	УстановитьДатуЗапрета('2018-02-28');
	
	// РТУ	
	Макет = ПолучитьМакет("РТУ_ЗакрытыйПериод");
	ТаблицаДанных = ОбработкаОбъект.ПолучитьТаблицуДанныхРеализацииТест(Макет);
	...
	Утверждения.ПроверитьРавенство(НачалоДня(Дата), '2018-03-01', "Не верная дата документа РТУ");
